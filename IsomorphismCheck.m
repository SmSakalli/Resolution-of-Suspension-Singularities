...
// _______________________ INITIAL RESOLUTION GRAPH _______________________________
...
// Add one "arrow" vertex per transverse intersection, attached to
// the vertex it comes from. Each arrow vertex has multiplicity 1.
...
// Given the initial resolution graph, one can construct the 
// corresponding resolution graph in terms of k, following Némethi's Algorithm.
...
// Converts a multigraph into a simple graph by subdividing
// repeated edges with a new intermediate vertex. 
//
// Used to be able to check isomorphism through MAGMA command 'IsIsomorphic()'
// which only takes Graph objects, as opposed to Multigraph objects.
//
// Original vertex labels are preserved.
...
SubdivideMultigraph := function(multigraph, mult_list, self_list, genus_list, arrow_list)
    ...
    // Copy labels from the original vertices.
    for i in [1..n] do
        l := [mult_list[i], self_list[i], genus_list[i], 0];
        AssignLabel(~G, VG[i], l);
    end for;

    // Keep track of which unordered vertex pairs have already
    // been connected so duplicate edges can be subdivided.
    seen_pairs := [];
    w := n;
    for i in [1..#E] do
        ...
        if pair in seen_pairs then
            // Replace additional parallel edges by a path
            // through a new subdivision vertex.
            w +:= 1;
            AddVertex(~G);
            VG := Vertices(G);
            ...
            // Keep the first occurrence of each edge unchanged.
...
// Compute and store the first 'period' subdivided graphs,
// which serve as the reference graphs for each isomorphism class.
first_round := [];
for i in [1..num] do
...
// Compare every later graph against the corresponding
// reference graph with the same value modulo 'period'.
...
// Report whether every comparison succeeded.
if isom_check then
    print "\nIsomorphisms HOLD for all k in [1..", finish, "] with period ", num;
else
