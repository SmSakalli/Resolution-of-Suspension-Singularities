...
// coeffients in its (negative) continued fraction decomposition
...
// For each edge [v_1,v_2], the appropriate set of internal vertices is found.
    ...
    //     [k_1,...,k_s]
    // that corresponds to the selfintersections of the new vertices iv_1,...,iv_s.
...
repeat
    ...
    for v in Alldeg(H,2) do
        if H_selfi[Index(v)] eq -1 and
           ...
           not HasParallelEdges(Index(v),H_edges) then
